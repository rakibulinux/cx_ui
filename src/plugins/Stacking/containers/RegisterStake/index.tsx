...
export const RegisterStake: React.FC<RegisterStakeProps> = (props: RegisterStakeProps) => {
    ...
    const handleSelectLockupPeriod = (period_index: number) => {
        ...
        const {period, annual_rate, min_amount, total_amount } = reward;
        setSelectedPeriodIndexState(period_index);
        console.log(period);
        
        setMinimumState(min_amount);
        setAnnualizedRewardState(Number(annual_rate) * 100);
        setStakingDateState((new Date().toDateString()));
        setLockupReleaseState((new Date().toDateString()));
        setExpectedRewardState(total_amount);
    }
